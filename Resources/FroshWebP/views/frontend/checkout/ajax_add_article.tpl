        ...
        <a href="{$detailLink}" class="link--article-image" title="{$sArticle.articlename|escape}">
            ...
            <span class="image--media">
                {if isset($image.thumbnails)}
                    ...
                    {/if}
                    <img data-srcset="{$image.thumbnails[0].sourceSet}" class="lazyload" alt="{$alt}" title="{$alt|truncate:160}" />
                {else}
                    {block name='frontend_detail_image_fallback'}
                        <img data-src="{link file='frontend/_public/src/img/no-picture.jpg'}" class="lazyload" alt="{$alt}" title="{$alt|truncate:160}" />
                    {/block}
                {/if}
            </span>
        </a>
